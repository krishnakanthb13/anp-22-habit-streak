...
// In-flight note UUID resolution promises to prevent concurrent duplicate note creation
const inFlightNoteResolutions = new Map();
const inMemoryUUIDCache = new Map();
 ...
 * serializes concurrent requests, and updates plugin settings automatically.
 *
...
export async function getNoteUUID(app, noteName, tagNames, settingKey) {
  ...
  const resolutionPromise = (async () => {
    // 1. Retrieve stored UUID from settings or in-memory cache
    let existingUUID = (app?.settings && typeof app.settings === "object")
      ? app.settings[settingKey] || inMemoryUUIDCache.get(settingKey) || null
      : inMemoryUUIDCache.get(settingKey) || null;
    ...
    if (existingUUID) {
      if (existingUUID.startsWith("local-")) {
        ...
        // Stored online UUID: Verify it actually exists before trusting it
        try {
    ...
    // 2. Search notes in-memory before creating a duplicate (Requires exact name AND matching tag)
    try {
      const allNotes = await app.filterNotes({});
      if (allNotes && Array.isArray(allNotes)) {
    ...
    // 3. Create new note and persist UUID in settings
    try {
...
export function extractJsonFromMarkdown(content) {
  ...
  // Prioritize matching ```json ... ``` code blocks, fallback to generic ``` ... ```
...
/**
 * Checks if raw note content represents an uninitialized or empty note rather than corrupted JSON data.
 * A note is uninitialized if it has no content, or only has markdown headings, titles, comments,
 * and contains NO code blocks (```) or JSON object braces ({ / }).
 * 
 * @param {string} content
 * @returns {boolean}
 */
export function isUninitializedContent(content) {
  ...
  // If the note has any code block fence, it was intended to have structured data
  if (trimmed.includes("```")) {
    return false;
  }

  // If it starts with braces or contains recognizable JSON keys, it is attempted JSON
  if (trimmed.startsWith("{") || trimmed.includes('"habits"') || trimmed.includes('"activeHabitId"')) {
    return false;
  }

  // Check if content is purely markdown headings (e.g. # habit_streak_data), blockquotes, or plain title lines
  const lines = trimmed.split("\n").map(l => l.trim()).filter(Boolean);
  const isAllHeadersOrBoilerplate = lines.every(line => 
    line.startsWith("#") || 
    line.startsWith(">") || 
    line.startsWith("-") || 
    line === DATA_NOTE_NAME || 
    line.toLowerCase() === "habit streak data" ||
    line.toLowerCase() === "habit_streak_data"
  );
 ...
 * - "empty": note was empty/new/uninitialized, initialized with default state
 * - "corrupt": note has damaged code block or broken JSON structure.
 *
...
export async function loadStateWithStatus(app) {
  try {
    ...
    // 1. If note has valid parseable JSON, return state
    if (content && typeof content === "string" && content.trim().length > 0) {
      ...
      // 2. If parsing failed, check if this is an uninitialized note (e.g. freshly created note with just title/header)
      ...
      // 3. CRITICAL INTEGRITY GUARD:
      // Persistent note content contains a damaged code block or broken JSON structure. Refuse destructive overwrite!
    ...
    // 4. Truly empty/new note: initialize with default state
    const fallbackState = normalizeState({ ...DEFAULT_STATE });
    const markdown = formatStateAsMarkdown(fallbackState);
    await app.replaceNoteContent({ uuid: dataNoteUUID }, markdown);
    return {
...
export function mutateState(app, mutator) {
  mutationQueue = mutationQueue
    ...
    .then(async () => {
      ...
      // CRITICAL INTEGRITY GUARD: Refuse to execute mutator or save if data note is corrupt or failed to load
