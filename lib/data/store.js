import { DATA_NOTE_NAME, DATA_NOTE_TAGS, DEFAULT_STATE, TRACK_TYPES, INTERVAL_PERIODS, generateUniqueId, COLOR_THEMES, VALID_THEMES } from "../constants.js";
...
/**
 * Validates whether a value is a valid parseable timestamp string.
 * @param {any} ts
 * @returns {boolean}
 */
...
export function normalizeHabit(habit) {
  ...
  const streakAnchor = (habit.streakAnchor && isValidTimestamp(habit.streakAnchor))
    ? habit.streakAnchor
    : nowISO;
  const streakStartedAt = (habit.streakStartedAt && isValidTimestamp(habit.streakStartedAt))
  ...
  const events = Array.isArray(habit.events)
    ? habit.events.filter(e => e && typeof e === "object" && typeof e.type === "string").map(e => ({
