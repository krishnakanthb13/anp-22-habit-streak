...
export function normalizeHabit(habit) {
  ...
  // Interval normalization (always daily)
  const interval = {
    n: 1,
...
export function isUninitializedContent(content) {
  ...
  // If it starts with braces or contains recognizable JSON schema keys, it is an attempted JSON payload
  if (trimmed.startsWith("{") || trimmed.includes('"habits"') || trimmed.includes('"activeHabitId"')) {
    return false;
  }

  // Any other non-code, non-JSON content (empty note, title heading, blank lines, boilerplate) is uninitialized
  return true;
}
