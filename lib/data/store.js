import { DATA_NOTE_NAME, DATA_NOTE_TAGS, DEFAULT_STATE, TRACK_TYPES, INTERVAL_PERIODS, generateUniqueId } from "../constants.js";
...
export function normalizeHabit(habit) {
  ...
  const trackingStartDate = (habit.trackingStartDate && typeof habit.trackingStartDate === "string")
    ? habit.trackingStartDate
    : (createdDateStr || nowISO.split("T")[0]);

  const streakAnchor = (habit.streakAnchor && typeof habit.streakAnchor === "string")
    ? habit.streakAnchor
    : nowISO;
  const streakStartedAt = (habit.streakStartedAt && typeof habit.streakStartedAt === "string")
    ? habit.streakStartedAt
    : streakAnchor;

  // Set arrays normalization (unique, string, sorted)
  const skips = Array.isArray(habit.skips)
    ? Array.from(new Set(habit.skips.filter(d => typeof d === "string" && d.length === 10))).sort()
    : [];

  const completions = Array.isArray(habit.completions)
    ? Array.from(new Set(habit.completions.filter(d => typeof d === "string" && d.length === 10))).sort()
    : [];

  const events = Array.isArray(habit.events)
    ? habit.events.filter(e => e && typeof e === "object")
    : [];

  const resetLogs = Array.isArray(habit.resetLogs)
    ? habit.resetLogs.filter(r => r && typeof r === "object")
    : [];
