...
/**
 * Calculates weekly repeatingness frequency of logs/events for the 7-day bar chart.
 * @param {object} habit
...
export function calculateWeeklyFrequency(habit, todayStr = getTodayString()) {
  ...
  for (let i = 6; i >= 0; i--) {
    ...
    // If no multi-events logged yet, fallback to single completion/skip count for the day
    if (count === 0) {
