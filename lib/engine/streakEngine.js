...
/**
 * Strictly validates that a string is in YYYY-MM-DD format and corresponds to a real calendar date.
 * @param {string} value
 * @returns {boolean}
 */
...
/**
 * Checks whether a given date is a scheduled tracking day for a habit.
 * Supports daily, every N days, weekly, and monthly recurrence.
 * If allowBackdated is true, checks whether dateStr aligns with the recurrence grid anchored at refStartStr.
 * @param {object} habit - Habit object.
 * @param {string} dateStr - YYYY-MM-DD
 * @param {string} [refStartStr] - Reference tracking start date (YYYY-MM-DD).
 * @param {boolean} [allowBackdated=false] - Whether to allow dates before habitStart if they align with the recurrence grid.
 * @returns {boolean}
 */
export function isScheduledDate(habit, dateStr, refStartStr, allowBackdated = false) {
  if (!isValidDateString(dateStr)) {
    return false;
  }

  const habitStart = (refStartStr && isValidDateString(refStartStr))
    ? refStartStr
    : (habit?.trackingStartDate && isValidDateString(habit.trackingStartDate)
      ? habit.trackingStartDate
      : (habit?.createdAt ? habit.createdAt.split("T")[0] : getTodayString()));
  ...
  const startDate = new Date(habitStart + "T00:00:00Z");
  ...
  const diffInDays = Math.round((targetDate.getTime() - startDate.getTime()) / (1000 * 60 * 60 * 24));
 ...
 * Prioritizes applicability (future -> before_start -> not_applicable) before checking manual records.
 * @param {object} habit - Habit object.
 * @param {string} dateStr - YYYY-MM-DD
 ...
 * @param {string} [effectiveStartStr] - Optional effective tracking start date.
 * @returns {"completed"|"skipped"|"not_applicable"|"future"|"before_start"}
 */
export function getHabitDayStatus(habit, dateStr, todayStr = getTodayString(), cachedSets = null, effectiveStartStr = null) {
  if (dateStr > todayStr) {
    return "future";
  }

  const habitStart = (effectiveStartStr && isValidDateString(effectiveStartStr))
    ? effectiveStartStr
    : (habit?.trackingStartDate && isValidDateString(habit.trackingStartDate)
      ? habit.trackingStartDate
      : (habit?.createdAt ? habit.createdAt.split("T")[0] : todayStr));

  if (dateStr < habitStart) {
    return "before_start";
  }

  // 1. First evaluate whether the date is scheduled according to habit interval
  const scheduled = isScheduledDate(habit, dateStr, habitStart);
  if (!scheduled) {
    return "not_applicable";
  }

  // 2. Date is scheduled: evaluate recorded completion / skip states
...
export function calculateHabitStats(habit, todayStr = getTodayString()) {
  ...
  let habitStart = habit.trackingStartDate && isValidDateString(habit.trackingStartDate)
    ? habit.trackingStartDate
    : (habit.createdAt ? habit.createdAt.split("T")[0] : todayStr);

  // Extend habitStart if there are earlier valid completed or skipped dates
  ...
  // Live Timer Anchor Timestamp: Verify date aligns with current streakStartDate
...
export function calculateWeeklyFrequency(habit, todayStr = getTodayString()) {
  ...
  for (let i = 6; i >= 0; i--) {
    ...
    const dayName = daysNames[d.getUTCDay()];
