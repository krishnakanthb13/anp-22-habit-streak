...
import { getTodayString, isValidDateString, isScheduledDate } from "../engine/streakEngine.js";
...
export async function handleToggleDay(app, habitId, dateStr, currentStatus) {
  ...
  try {
    ...
    await mutateState(app, async state => {
      ...
      const scheduleAnchor = (habit.trackingStartDate && isValidDateString(habit.trackingStartDate))
        ? habit.trackingStartDate
        : (habit.createdAt ? habit.createdAt.split("T")[0] : dateStr);

      // Mutation Guard: Validate against the fixed schedule anchor first!
      if (!isScheduledDate(habit, dateStr, scheduleAnchor, true)) {
        return;
      }

      // Mutate the start date invariant ONLY after validation passes
      ...
      // Determine current status: use currentStatus hint or inspect habit records
 ...
 * Filters out non-scheduled dates from being persisted.
 * @param {object} app - Amplenote App instance.
 * @param {string} habitId - Habit ID.
...
export async function handleSaveCalendarEdits(app, habitId, skips, completions) {
  ...
  try {
    await mutateState(app, async state => {
      ...
      // Validate against the fixed schedule anchor first!
      const validSkips = Array.isArray(skips)
        ? Array.from(new Set(skips.filter(d => isValidDateString(d) && isScheduledDate(habit, d, scheduleAnchor, true)))).sort()
        : [];
      const skipSet = new Set(validSkips);

      const rawCompletions = Array.isArray(completions)
        ? Array.from(new Set(completions.filter(d => isValidDateString(d) && isScheduledDate(habit, d, scheduleAnchor, true)))).sort()
        : [];
