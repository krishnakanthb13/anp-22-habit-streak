...
export async function handleToggleDay(app, habitId, dateStr, currentStatus) {
  ...
  try {
    ...
    await mutateState(app, async state => {
      ...
      const currentStart = (habit.trackingStartDate && isValidDateString(habit.trackingStartDate))
      ...
      // Mutation Guard: Do not allow toggling non-scheduled / off-days
...
export async function handleSaveCalendarEdits(app, habitId, skips, completions) {
  ...
  try {
    await mutateState(app, async state => {
      ...
      // Determine effective trackingStartDate if incoming dates are backdated
      const allIncomingDates = [...(Array.isArray(skips) ? skips : []), ...(Array.isArray(completions) ? completions : [])].filter(isValidDateString).sort();
      let effectiveStart = (habit.trackingStartDate && isValidDateString(habit.trackingStartDate))
        ? habit.trackingStartDate
        : (habit.createdAt ? habit.createdAt.split("T")[0] : (allIncomingDates[0] || getTodayString()));
      ...
      const validSkips = Array.isArray(skips)
        ? Array.from(new Set(skips.filter(d => isValidDateString(d) && isScheduledDate(habit, d, effectiveStart)))).sort()
        : [];
      const skipSet = new Set(validSkips);

      const rawCompletions = Array.isArray(completions)
        ? Array.from(new Set(completions.filter(d => isValidDateString(d) && isScheduledDate(habit, d, effectiveStart)))).sort()
        : [];
