...
export async function handleUndoToday(app, habitId) {
  ...
  try {
    ...
    await mutateState(app, async state => {
      ...
      // 1. Remove only the latest event for today if present
      ...
      // 2. Remove latest resetLog for today if applicable
      ...
      // 3. Reconstruct today's date state from remaining today's events
      ...
      if (remainingTodayEvents.length > 0) {
        ...
        // No remaining events today: remove from both arrays to restore default state
        habit.skips = habit.skips.filter(d => d !== todayStr);
        habit.completions = habit.completions.filter(d => d !== todayStr);
      }
    });
